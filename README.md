...
## Repository Contents
The goal of this project is to use Latent Dirichlet Allocation (LDA) to understand the most common themes in positive and negative McDonald's restaurant reviews to pinpoint areas most in need of improvement. The DS-4002_Group5_Project1 repository contains the DATA folder (includes 3 versions of our dataset of interest with more information located in the data appendix), the SCRIPTS folder (includes code for preprocessing/cleaning the dataset, fitting the LDA model and assigning topics to reviews, and performing a hypothesis test), the OUTPUTS folder (contains screenshots or PDFs of our results from the LDA model and the hypothesis test), and the LICENSE.md and READ.md files.
...
### Folder Descriptions
- **DATA**: Contains 3 datatsets and the data appendix that includes more information about each dataset.
  - Original dataset of 33,396 McDonald's Reviews downloaded from Kaggle (scraped from Google)
  - Cleaned dataset after preprocessing
  - Cleaned dataset with additional columns added for topics, topic probabilities, and sentiments assigned to each review 
- **SCRIPTS**: Contains python scripts for data preprocessing, LDA modeling, and hypothesis testing.
...
- **Step 3**: After preprocessing the data, run the 02_doc_term_matrix_and_LDA.py script.
  - This script will:
    ...
    - Evaluate mutiple k values based on coherence scores to find the most optimal k value for the LDA model
  ...
  - Some notes for Step 3:
    ...
    - While the coherence score for k=3 was the highest, k=6 was chosen because it was the highest k value before a noticeable dip in coherence when k >= 7. A higher k value implies more topics existing throughout the data and provides more interesting results. In summary, k=6 was a balance between a good coherence score and producing interesting/interpretable results.
...
- **Step 4**: For the final step of the analysis, run the 03_hypothesis_test.py script. The script performs two one-proportion z-tests to test whether at least 30% of positive and negative reviews are assigned to food quality and bad service, respectively. It also reports the distribution of the 6 topic labels by sentiment. The z-test conclusions indicate whether food quality and customer service at at least as prevalent as the 30% benchmark.
